...
public class PaymentCrudAction extends CrudActionBase<PaymentDTOEx> {
    ...
    // bank
    private static final String FIELD_GROUP_BANK_DATE = "bankDate";
    private static final String FIELD_BANK_PAID_TO = "paidTo";
    private static final String FIELD_BANK_REF = "bankRef";
    ...
    protected ForwardAndMessage doSetup() {
        ...
        // added bank info
        if (bankDto != null) {
            myForm.set(FIELD_BANK_PAID_TO, bankDto.getPaidTo());
            myForm.set(FIELD_BANK_REF, bankDto.getBankRef());
            setFormDate(FIELD_GROUP_BANK_DATE, bankDto.getDate());
        }
    ...
    protected PaymentDTOEx doEditFormToDTO() {
        ...
        } else if ("bank".equals(payMethod)) { // added Bank
            // create the Bank Info dto
            PaymentInfoBankDTO bankDto = new PaymentInfoBankDTO();
            bankDto.setPaidTo((String) myForm.get(FIELD_BANK_PAID_TO));
            bankDto.setBankRef((String) myForm.get(FIELD_BANK_REF));
            bankDto.setDate(parseDate(FIELD_GROUP_BANK_DATE, "payment.cheque.date"));
            // set the bank
            dto.setBank(bankDto);
            dto.setPaymentMethod(new PaymentMethodDTO(Constants.PAYMENT_METHOD_BANK));
            // validate required fields        
            required(bankDto.getPaidTo(), "payment.bank.paidto");
            required(bankDto.getBankRef(), "payment.bank.ref");
            required(bankDto.getDate(), "payment.bank.date");
            // Bank payments are never process realtime 
            dto.setPaymentResult(new PaymentResultDTO(Constants.RESULT_ENTERED));
            session.setAttribute("tmp_process_now", false);
